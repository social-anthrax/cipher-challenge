...
int main(int argc, char *argv[])
{
        ...
        char cipher[]  = "";
        ...
        for (i = 0; i < len; i += 2) {
                if(a == b)
                {
                        ...
                        if(a_col == 0)
                                ...
                                a_col--;
                        result[i] = key[a_row*5 + a_col];
                        result[i+1] = key[a_row*5 + a_col];
                        continue;
                }
                a = text[i];
                b = text[i+1];
